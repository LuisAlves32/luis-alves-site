// Página original (21st.dev): https://21st.dev/@rmahammad/components/border-beam-panel
// Licença: MIT, Copyright (c) 2026 Mahammad Rustamov. Créditos completos em CREDITOS.md.
"use client";

/* Origem: 21st.dev, "Border Beam Panel" de rmahammad (demo 23408), escolhido
   pelo diretor para o cartão de credibilidade do bloco 3 da home. Instalado
   COMO ESTÁ, de propósito: o componente já resolve sozinho o que costuma
   faltar nesse tipo de efeito. Tem estado estático sob prefers-reduced-motion,
   pausa quando sai da tela e quando a aba perde o foco, ângulo inicial
   determinístico (não quebra hidratação), respeita forced-colors e não traz
   dependência nenhuma. Posse do movimento: o rAF daqui de dentro.

   TRÊS MUDANÇAS, e só três, todas para o `npm run lint` deste projeto passar
   (o eslint do Next 16 reprova três padrões que o autor usou). Nenhuma delas
   muda comportamento; cada uma está comentada no ponto:
   1. `useReducedMotion` não reescreve mais o estado logo na entrada do efeito;
   2. a marca de hidratação virou useSyncExternalStore, no lugar de useState
      mais setState num efeito;
   3. o liveRef passou a ser escrito num efeito, e não durante o render.

   A passada de tokens da marca (design.md) acontece no consumo, por className
   e por props (Avanço e o azul claro das portas, raio 12px, velocidades mais
   lentas que o padrão do autor). O bloco MOTIQ_TOKENS abaixo fica numa camada
   de cascata baixa, então os tokens do projeto sempre ganham dele; e o bloco
   .dark é inerte neste site (a classe nunca é aplicada). */
...
function useReducedMotion(): boolean {
  ...
  React.useEffect(() => {
    ...
    // O valor inicial já veio do inicializador acima, que roda no cliente e lê
    // a mesma media query: reescrevê-lo aqui era setState síncrono dentro de
    // efeito (react-hooks/set-state-in-effect) sem mudar nada. Só a assinatura
    // de mudanças fica.
...
/** Subscrição inerte: só serve para o useSyncExternalStore abaixo. */
const noSubscribe = () => () => {};
...
function BorderBeamPanelBase({
  ...
  // (era useState + setState num efeito; useSyncExternalStore dá o mesmo
  ...
  // Antes era escrito durante o render (react-hooks/refs). Como o ref só é
  // LIDO em manipulador de evento e em efeito, escrevê-lo num efeito declarado
  // antes dos outros dá o mesmo resultado.
  React.useEffect(() => {
